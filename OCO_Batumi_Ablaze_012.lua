...
-- get player status updates (e.g. death)
function PlayerStatus(args, time)
    -- get player units
    local unit_slick1 = Unit.getByName("Slick1")
    local unit_slick2 = Unit.getByName("Slick2")
    local unit_slick3 = Unit.getByName("Slick3")
    local unit_slick4 = Unit.getByName("Slick4")
    local player_units = {unit_slick1, unit_slick2, unit_slick3, unit_slick4}

    --if Life is <1 DCS considers Unit dead
    for i=1,#player_units do
        if(player_units[i].getLife() < 1) then
            trigger.action.outSoundForCoalition(1, "helo_down.wav")
        end
    end
...
do
    ...
    timer.scheduleFunction(PlayerStatus, nil, timer.getTime() + 1)
