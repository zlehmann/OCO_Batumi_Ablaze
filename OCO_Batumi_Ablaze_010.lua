...
function UnitRadioCommand(unitName)
    ...
    if #UnitStateTable[unitName] > 0 then
        ...
        if dropoffZone ~= nil then
            ...
            --local newGroup = dropoffZone.DropFunction(UnitStateTable[unitName], "dynBlue", 15, unitpos.x, unitpos.z, destinationZone.point.x, destinationZone.point.z)
...
function SpawnNewTrucks(args, time)
    ...
    if(trigger.misc.getUserFlag(98) ~= 1) then
        ...
        for i=1,difficulty_modifier do
            ...
            coalitionResources[2].supply = coalitionResources[2].supply - 5    
            
            mist.scheduleFunction(mist.groupToRandomZone, {red_clone["name"], destZones, 'cone', nil, 40, false}, timer.getTime() + 30)
        end
    end
